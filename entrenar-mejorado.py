...
class PreprocessorAvanzado:
    ...
    def detectar_y_corregir_outliers(self, df, columna='Precip_mm', metodo='iqr'):
        """
        🔥 SOLUCIÓN 3 IMPLEMENTADA: Límites MÁS PERMISIVOS
        ...
        if metodo == 'iqr':
            ...
            # 🔥 CAMBIO: 5*IQR en vez de 3*IQR → MÁS PERMISIVO
            # Esto preserva más variabilidad natural
            ...
            # 🔥 CAMBIO: 99.5 en vez de 99 → MÁS PERMISIVO
            p995 = serie.quantile(0.995)
            serie_corregida = serie.clip(upper=p995)
...
class ModeloHibridoAQUARIA:
    ...
    def crear_lstm_mejorado(self, input_shape):
        """
        🔥 SOLUCIÓN 2 IMPLEMENTADA: MENOS regularización para capturar variabilidad
        LSTM con arquitectura optimizada para series temporales
        - Menos capas pero más neuronas
        - Dropout adaptativo REDUCIDO
        - Batch Normalization para estabilidad
        """
        model = keras.Sequential([
            # 🔥 Capa 1: MENOS regularización (dropout 0.15 vs 0.2, l2 0.0001 vs 0.001)
            keras.layers.Bidirectional(
                keras.layers.LSTM(
                    128, 
                    return_sequences=True,
                    dropout=0.15,  # 🔥 Reducido de 0.2
                    recurrent_dropout=0.05,  # 🔥 Reducido de 0.1
                    kernel_regularizer=keras.regularizers.l2(0.0001)  # 🔥 10x más suave
                ),
                input_shape=input_shape
            ),
            keras.layers.BatchNormalization(),
            
            # 🔥 Capa 2: MENOS regularización
            keras.layers.Bidirectional(
                keras.layers.LSTM(
                    96,
                    return_sequences=True,
                    dropout=0.15,  # 🔥 Reducido de 0.2
                    recurrent_dropout=0.05,  # 🔥 Reducido de 0.1
                    kernel_regularizer=keras.regularizers.l2(0.0001)  # 🔥 Más suave
                )
            ),
            keras.layers.BatchNormalization(),
            
            # 🔥 Capa 3: MENOS regularización
            keras.layers.Bidirectional(
                keras.layers.LSTM(
                    64,
                    dropout=0.1,  # 🔥 Reducido de 0.15
                    recurrent_dropout=0.05,  # 🔥 Reducido de 0.1
                    kernel_regularizer=keras.regularizers.l2(0.0001)  # 🔥 Más suave
                )
            ),
        ...
        # 🔥 CAMBIO CRÍTICO: MSE en vez de Huber
        # Huber es demasiado robusto y suaviza las predicciones
        model.compile(
            optimizer=optimizer,
            loss='mse',  # 🔥 Cambio de Huber a MSE estándar
            metrics=['mae', 'mse']
        )
    ...
    def entrenar(self, df_combined, columna_target='Precip_mm'):
        ...
        # 🔥 SOLUCIÓN 4 IMPLEMENTADA: POST-PROCESAMIENTO DE VARIABILIDAD
        print(f"\n  🎯 Post-procesamiento de variabilidad...")
        
        # Calcular variabilidad actual
        variability_ratio_inicial = np.std(y_pred_hybrid) / (np.std(y_test) + 1e-6)
        print(f"    Variabilidad inicial: {variability_ratio_inicial:.3f}")
        
        if variability_ratio_inicial < 0.75:  # Si es muy baja
            print(f"    ⚠️  Aplicando corrección de variabilidad...")
            
            target_ratio = 0.85  # Objetivo: 85% de variabilidad real
            mean_pred = np.mean(y_pred_hybrid)
            
            # Amplificar desviaciones de la media
            y_pred_hybrid = mean_pred + (y_pred_hybrid - mean_pred) * (target_ratio / variability_ratio_inicial)
            y_pred_hybrid = np.maximum(y_pred_hybrid, 0)
            
            variability_ratio_final = np.std(y_pred_hybrid) / (np.std(y_test) + 1e-6)
            print(f"    ✓ Variabilidad corregida: {variability_ratio_inicial:.3f} → {variability_ratio_final:.3f}")
    ...
    def _optimizar_alpha(self, y_true, y_pred_lstm, y_pred_sarimax):
        """
        🔥 SOLUCIÓN 1 IMPLEMENTADA: Optimiza alpha balanceando MAE y variabilidad
        Encuentra el mejor peso para combinar LSTM y SARIMAX
        """
        mejor_alpha = 0.5
        mejor_score = float('inf')
        
        # Variabilidad objetivo (del test set real)
        target_std = np.std(y_true)
        
        print(f"    Buscando alpha óptimo (target_std={target_std:.2f}mm)...")
        
        for alpha in np.arange(0.0, 1.01, 0.05):
            y_pred = alpha * y_pred_lstm + (1 - alpha) * y_pred_sarimax
            
            # 🔥 MÉTRICA COMBINADA: MAE + penalización de variabilidad
            mae = mean_absolute_error(y_true, y_pred)
            
            # Penalización si la variabilidad es muy diferente de la real
            pred_std = np.std(y_pred)
            variability_penalty = abs(pred_std - target_std) / (target_std + 1e-6)
            
            # Score combinado (70% MAE, 30% variabilidad)
            # Ajusta el peso 0.4 si quieres dar más o menos importancia a variabilidad
            combined_score = mae * (1.0 + 0.2 * variability_penalty)
            
            if combined_score < mejor_score:
                mejor_score = combined_score
                mejor_alpha = alpha
                
                # Mostrar solo las mejoras
                if alpha == 0.0 or mejor_score < float('inf'):
                    print(f"      α={alpha:.2f}: MAE={mae:.2f}, std={pred_std:.2f}, score={combined_score:.2f} ✓")
...
def entrenar_modelo_grupo_hibrido(nombre_grupo, config_grupo, carpeta_precip, carpeta_meteo):
    ...
    metadata = {
        ...
        'version': '2.1_variabilidad_mejorada',  # 🔥 Nueva versión
...
def main():
    print("="*70)
    print("🌊 AQUARIA - ENTRENAMIENTO MODELO HÍBRIDO MEJORADO v2.1")
    print("🔥 CON SOLUCIONES DE VARIABILIDAD IMPLEMENTADAS")
    print("="*70)
    ...
    print(f"\n💡 SIGUIENTE PASO: Ejecuta 'python calibrar_simple.py' para regenerar calibradores")
