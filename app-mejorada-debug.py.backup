...
DEBUG = True  # 🔥 Activar para diagnóstico
    ...
    for archivo in archivos_antiguos:
        ...
        if nombre not in calibradores:
            ...
            try:
                ...
                # Validar estructura (debe tener 'factores')
                if isinstance(calibrador, dict) and 'factores' in calibrador:
                    calibradores[nombre] = calibrador
                    if DEBUG:
                        print(f"     ✅ Cargado como '{nombre}'")
                        print(f"     → Tipo: Multiplicativo")
                else:
                    if DEBUG:
                        print(f"     ⚠️  Estructura inválida (falta 'factores')")
            except Exception as e:
                if DEBUG:
                    print(f"     ❌ Error: {e}")
        else:
...
def cargar_todos_modelos():
    """Carga modelos Y calibradores - con manejo robusto de errores"""
    modelos = {}
    ...
    for archivo in Path('modelos').glob('metadata_*.pkl'):
        ...
        # Saltar modelos híbridos antiguos si existen
        ...
        try:
            ...
            if DEBUG:
                ...
                print(f"✅ Modelo cargado: {nombre} | Calibrador: {cal_status}")
            
        except Exception as e:
            if DEBUG:
                print(f"❌ Error cargando {nombre}: {e}")
            # No mostrar en sidebar para evitar ruido
            continue
...
def aplicar_calibracion_estratificada(predicciones, calibrador, modo='balanceado'):
    """
    Aplica calibración estratificada por nivel (MÉTODO CORRECTO)
    """
    import numpy as np
    ...
    # Paso 1: Aumentar variabilidad (opcional)
    if factor_var > 1.0:
        ...
        with col2:
            if usar_calibracion:
                modo_calibracion = st.radio(
                    ...
                    index=1 if es_estratificado else 0,  # Balanceado por defecto para estratificado
                    help="""
                    • Conservador: Corrección suave (mejor para alertas tempranas)
                    • Balanceado: Corrección moderada (uso general recomendado)
                    • Agresivo: Corrección fuerte (predicciones más ajustadas)
                    """
                )
...
with tab3:
    ...
    if calibrador_actual:
        ...
        st.info("""
        📊 **Información del Calibrador Entrenado**
        
        Este calibrador fue generado analizando las últimas 12 semanas de validación histórica
        para detectar y corregir sesgos sistemáticos del modelo.
        """)
